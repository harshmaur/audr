...
func TestRule_ChromeDevToolsMCPDaemonPidSymlink(t *testing.T) {
	cases := []struct {
		...
		{
			name: "vulnerable npx package fires",
			path: "/test/.cursor/mcp.json",
		...
		},
		{
			...
			path: "/test/.codex/config.toml",
			body: `[mcp_servers.devtools]` + "\n" + `command = "npx"` + "\n" + `args = ["npm:chrome-devtools-mcp@0.20.0"]`,
			want: true,
		},
		{
			name: "fixed version does not fire",
		...
		},
		{
			...
			path: "/test/.cursor/mcp.json",
			body: `{"mcpServers":{"devtools":{"command":"npx","args":["chrome-devtools-mcp@0.19.9"]}}}`,
			want: false,
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			doc := parse.Parse(c.path, []byte(c.body))
			if got := fired(doc, "chrome-devtools-mcp-daemon-pid-symlink"); got != c.want {
				t.Errorf("fired = %v, want %v (rules: %v)", got, c.want, applyRule(doc))
			}
		})
	}
