...
func TestOpenClawTrustedProxyScopeClearing_FlagsVulnerablePackage(t *testing.T) {
	doc := parse.Parse("package.json", []byte(`{"name":"openclaw","version":"2026.3.30"}`))
	findings := (openclawTrustedProxyScopeClearing{}).Apply(doc)
	if len(findings) != 1 {
		t.Fatalf("got %d findings, want 1", len(findings))
	}
	if findings[0].RuleID != "openclaw-trusted-proxy-scope-clearing" {
		t.Fatalf("rule id = %q", findings[0].RuleID)
	}
}

func TestOpenClawTrustedProxyScopeClearing_FlagsVulnerableDependency(t *testing.T) {
	doc := parse.Parse("package.json", []byte(`{"dependencies":{"openclaw":"^2026.3.24"}}`))
	findings := (openclawTrustedProxyScopeClearing{}).Apply(doc)
	if len(findings) != 1 {
		t.Fatalf("got %d findings, want 1", len(findings))
...
func TestOpenClawTrustedProxyScopeClearing_AllowsFixedVersion(t *testing.T) {
	doc := parse.Parse("package.json", []byte(`{"name":"openclaw","version":"2026.3.31","dependencies":{"openclaw":"2026.4.1"}}`))
	findings := (openclawTrustedProxyScopeClearing{}).Apply(doc)
	if len(findings) != 0 {
		t.Fatalf("got %d findings, want 0", len(findings))
