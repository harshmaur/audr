...
func shouldDescendAmazonInspectorNPMMalwarePath(relSlash string) bool {
	...
	if len(parts) >= 3 && parts[0] == ".pnpm" && parts[2] == "node_modules" {
		if len(parts) == 3 {
			for _, prefix := range []string{
				...
				"api-node-sdk@", "app-soda-layer@", "sigchain-js@", "chain-analyze@", "react-puller@", "claude-remote-agent@", "llm-interceptor@", "map-streak-kit@", "streak-map-kit@", "kit-vim-map@", "kit-map-vim@", "dim-hydration-ui@", "w-screenctl@", "aclade-agent@", "agenthub-ai@", "uibabai@", "simple-date-formatter-new-9@", "simple-date-formatter-new-10@", "tokocrytodev@", "cryptostock@", "notafollower@", "depcruise-wrap-stream-in-html@", "pfp-forms-sme-loan@", "checkout-desktop-total@", "bcc-design@", "bcc-design-icons@", "setup-codex@", "expect-dotenv@", "mcp-dev-toolkit@", "express-session-handler@", "chai-as-soul@", "chai-as-otc@", "chai-as-org@", "spotify-url-infos@", "spotify-url-resolvers@", "octopus-action@", "mt-ts-serverless-starter@", "fuel-react@", "lumen-pages-community@", "@gfe+lx-watcher@", "@httttt+mcp-demo@", "@guangnao+agent-proxy@", "@yancyyu+agentcli@", "@js-lib-team+env-parser@",
			} {
	...
	case "chalk-utils", "joi-pack", "rimraf-utils", "nock-helper",
		...
		"simple-date-formatter-new-9", "simple-date-formatter-new-10", "tokocrytodev", "cryptostock", "notafollower", "depcruise-wrap-stream-in-html", "pfp-forms-sme-loan", "checkout-desktop-total", "core-tailwindcss-utility", "bcc-design", "bcc-design-icons", "mcp-dev-toolkit", "express-session-handler", "spotify-url-infos", "spotify-url-resolvers", "octopus-action", "mt-ts-serverless-starter", "fuel-react", "lumen-pages-community":
		return len(parts) == 1
	...
	case "@js-lib-team":
		return len(parts) == 1 || (len(parts) == 2 && parts[1] == "env-parser")
	case "@httttt":
		if len(parts) == 1 {
			return true
		}
		return parts[1] == "mcp-demo" &&
			(len(parts) == 2 || (len(parts) == 3 && parts[2] == "dist"))
	case "map-streak-kit", "streak-map-kit", "kit-vim-map", "kit-map-vim", "dim-hydration-ui":
		return len(parts) == 1 ||
			(len(parts) == 2 && parts[1] == "dist") ||
			(len(parts) == 3 && parts[1] == "dist" && parts[2] == "internal")
	case "@yancyyu":
		if len(parts) == 1 {
			return true
		}
		return parts[1] == "agentcli" && (len(parts) == 2 || (len(parts) == 3 && parts[2] == "bin"))
	default:
		return false
	}
}
